...
PageNo Cell::leftChildPageNo() const {
  if (pimpl->page.isLeaf())
    throw std::runtime_error("Left child page no is valid only for cells in interior pages");
...
PayloadSize Cell::payloadSize() const {
  if (pimpl->page.isTable()) {
    if (pimpl->page.isLeaf())
      return *reinterpret_cast<PayloadSize*>
    else
      throw std::runtime_error("Payload size is not valid for interior table pages");
  if (pimpl->page.
}
...
Page::Page(Table& table, std::unique_ptr<char[]> raw_data, PageNo parent_page_no) {
}
...
Table::Table(Database& database, std::string name, std::istream file, PageNo root_page_no, RowId next_row_id, PageSize page_size) {
}
...
static Page Table::createInteriorPage(PageNo page_no, PageNo rightmost_child_page_no, PageNo parent_page_no) {
}

static Page Table::createLeafPage(PageNo page_no, PageNo right_sibling_page_no, PageNo parent_page_no) {
}
